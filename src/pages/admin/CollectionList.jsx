...
export default function CollectionList() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await del('/api/collections/' + id); loadData(); } catch { /* 错误已由 request.js 统一提示 */ }
  };
    ...
    <div>
      <div className="navbar">
        <div className="navbar-left">
          ...
          <span className="navbar-title">藏品管理</span>
        </div>
        <button className="btn btn-sm" style={{ background: 'rgba(255,255,255,0.15)', color: '#fff' }} onClick={() => navigate('/admin/collection-form')}>+ 新增</button>
      </div>
      ...
      <div className="container" style={{ paddingTop: 12 }}>
          ...
          <>
            ...
            <div className="sort-panel" style={{ position: 'fixed', top: '30%', left: 20, right: 20, background: '#fff', borderRadius: 14, padding: 8, zIndex: 1001, boxShadow: '0 12px 40px rgba(0,0,0,0.2)' }}>
              {SORT_OPTIONS.map((opt, i) => (
                <div key={i} className={`sort-item ${i === sortIdx ? 'active' : ''}`}
                  style={{ display: 'flex', justifyContent: 'space-between', padding: '14px 20px', cursor: 'pointer', borderBottom: i < SORT_OPTIONS.length - 1 ? '1px solid #f0f0f0' : 'none', color: i === sortIdx ? 'var(--brown-dark)' : 'var(--text)', fontWeight: i === sortIdx ? 600 : 400 }}
        ...
        {list.length < total && (
          <div className="load-more">
            <button className="btn btn-default" onClick={() => loadData(true)}>加载更多</button>
          </div>
        )}

        {list.length === 0 && (
          <div className="empty" style={{ textAlign: 'center', padding: 60, color: 'var(--brown-fade)' }}>
